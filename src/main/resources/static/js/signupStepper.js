...
function emailValidation() {
	...
	if (!email.match(/^[A-Za-z0-9]{1}[A-Za-z0-9_.-]*@{1}[A-Za-z0-9_.-]+.[A-Za-z0-9]+$/)) {
		$('.error-mail').text(`メールアドレスは xxx@yyy.zzz 形式で入力してください`);
		return false;
	}
	$('.error-mail').text(``);
...
function telValidation() {
	...
	if (!tel.match(/^[0-9]{3}-[0-9]{4}-[0-9]{4}$/)) {
		$('.error-telephone').text(`電話番号はXXXX-XXXX-XXXXの形式で入力してください`);
		return false;
	}
...
function passwordValidation() {
	...
	if (!password.match(/^[a-z\\d]{8,16}$/)) {
		$('.error-password').text(`パスワードは8文字以上１６文字以下で入力してください`);
		return false;
	}
	$('.error-password').text(``);
	return true;
...
function confirmPasswordValidation() {
	...
	if (password !== confirmPassword) {
		$('.error-confirmPassword').text(`パスワードと確認用パスワードが不一致です`);
		return false;
	}
